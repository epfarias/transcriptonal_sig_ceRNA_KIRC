...
enrichBubblePlotFun <- function(kegg, pval=0.05) {
  keggBasic = ggplot(data=kegg, mapping=aes(x=Terms, 
                                            y=foldEnrichment,color=pValue,size=Counts))
...
enrichBarPlotFun <- function(kegg, type='single', bar.color='black') {
  ...
  if (type=='single') {
    keggBasic = ggplot(data=kegg, mapping=aes(x=Terms, y=-log(pValue,10)))
    keggBasic + geom_bar(stat='identity',fill=bar.color) + 
      scale_x_discrete(limits=rev(kegg$Terms)) + 
      ylim(0, max(-log(kegg$pValue,10))) + 
      theme(legend.title=element_blank())+ylab('-log10(pValue)')+
      xlab('') + coord_flip() + 
      theme_bw()+theme(axis.line = element_line(colour = "black"),
    ...
    keggBasic + geom_bar(stat='identity') + 
      ...
      ylab('-log10(pValue)')+xlab('') + coord_flip() + 
